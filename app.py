...
<style>
    ...
    .metric-detail { font-size: 0.78rem; color: #4A4540; line-height: 1.5; }
    .section-header { font-size: 1.1rem; font-weight: 500; color: #1E1B18; margin-bottom: 0.25rem; }
    .section-desc { font-size: 0.82rem; color: #9A9088; margin-bottom: 1rem; line-height: 1.6; }
...
def generate_population(n=2000, seed=42):
    ...
    risk_score = (
        (age / 100) * 20 + chronic_conditions * 12 +
        (bmi > 30).astype(int) * 8 + smoking * 10 +
        ed_visits_12m * 5 + (1 - medication_adherence) * 15 +
        social_isolation * 6 + housing_instability * 8 +
        food_insecurity * 6 + transportation_barrier * 4 +
        (income_level == "Low").astype(int) * 8 +
        (insurance_type == "Uninsured").astype(int) * 10 +
        np.random.normal(0, 5, n)
    ).clip(0, 100)
    hosp_prob = (
        0.01 + (risk_score / 100) * 0.45 +
    ...
    df = pd.DataFrame({
        ...
        "bmi": bmi.round(1), "smoking": smoking, "ed_visits_12m": ed_visits_12m,
        "primary_care_visits": primary_care_visits,
        "medication_adherence": medication_adherence.round(2),
        "social_isolation": social_isolation, "housing_instability": housing_instability,
        "food_insecurity": food_insecurity, "transportation_barrier": transportation_barrier,
        "risk_score": risk_score.round(1), "hosp_prob": hosp_prob.round(3),
...
def run_clustering(df):
    features = ["age", "chronic_conditions", "bmi", "ed_visits_12m", "medication_adherence", "risk_score"]
    X_scaled = StandardScaler().fit_transform(df[features].copy())
    df = df.copy()
    df["segment"] = KMeans(n_clusters=4, random_state=42, n_init=10).fit_predict(X_scaled)
    seg_risk = df.groupby("segment")["risk_score"].mean().sort_values()
    df["segment"] = df["segment"].map({old: new for new, old in enumerate(seg_risk.index)})
    df["segment_name"] = df["segment"].map({
        0: "Segment A: Healthy", 1: "Segment B: Moderate Risk",
...
def train_model(df):
    ...
    X_train, _, y_train, _ = train_test_split(df[features], df["hospitalized"], test_size=0.2, random_state=42)
    model = GradientBoostingClassifier(n_estimators=100, random_state=42)
    model.fit(X_train, y_train)
    labels = {
        "age": "Age", "chronic_conditions": "Chronic Conditions", "bmi": "BMI",
        "smoking": "Smoking Status", "ed_visits_12m": "ED Visits (12 months)",
        "medication_adherence": "Medication Adherence", "social_isolation": "Social Isolation",
    ...
    importance["feature_label"] = importance["feature"].map(labels)
    return model, importance

# ── Colors ────────────────────────────────────────────────────────────────────

COLORS = {"terracotta": "#C4714A", "sage": "#7A8C72", "blush": "#C4908A",
          "gold": "#C8A86A", "sky": "#7AA0B8", "ink": "#1E1B18"}

RISK_COLORS = {"Low": "#7A8C72", "Moderate": "#C8A86A", "High": "#D4896A", "Critical": "#C4714A"}

SEGMENT_COLORS = {
    "Segment A: Healthy": "#7A8C72", "Segment B: Moderate Risk": "#C8A86A",
    "Segment C: High Risk": "#D4896A", "Segment D: Critical": "#C4714A",
}

AXIS_STYLE = dict(
    tickfont=dict(color="#1E1B18", size=12),
    title_font=dict(color="#1E1B18", size=12),
)

PLOTLY_LAYOUT = dict(
    paper_bgcolor="rgba(0,0,0,0)",
    plot_bgcolor="#FDFAF7",
    font=dict(family="DM Sans", color="#1E1B18", size=12),
    title_font=dict(family="DM Sans", color="#1E1B18", size=14),
    legend=dict(font=dict(color="#1E1B18", size=11)),
...
with st.sidebar:
    ...
    selected_risk = st.multiselect("Risk Tier", ["Low", "Moderate", "High", "Critical"],
                                   default=["Low", "Moderate", "High", "Critical"])
    selected_insurance = st.multiselect("Insurance Type",
                                        ["Medicaid", "Medicare", "Commercial", "Uninsured"],
                                        default=["Medicaid", "Medicare", "Commercial", "Uninsured"])
    st.markdown("---")
    st.markdown("""<div style='font-size: 0.75rem; color: #9A9088; line-height: 1.6;'>
...
st.markdown("""<div style='margin-bottom: 0.5rem;'>
    ...
    text-transform: uppercase; color: #C4714A;'>Population Health Analytics · Synthetic Patient Data</span>
</div>""", unsafe_allow_html=True)

st.title("Population Health Risk Stratifier")
st.markdown("""<p style='color: #4A4540; font-size: 0.88rem; max-width: 700px; line-height: 1.7; margin-bottom: 1.5rem;'>
...
with col2:
    high_pct = df_filtered["risk_tier"].isin(["High", "Critical"]).sum() / len(df_filtered) * 100
    st.markdown(f"""<div class='metric-card'>
        <div class='metric-label'>High or Critical Risk</div>
        <div class='metric-num'>{high_pct:.1f}%</div>
        <div class='metric-detail'>{df_filtered["risk_tier"].isin(["High", "Critical"]).sum():,} patients</div>
    </div>""", unsafe_allow_html=True)
with col3:
    st.markdown(f"""<div class='metric-card sage'>
        <div class='metric-label'>Avg Risk Score</div>
        <div class='metric-num' style='color: #7A8C72;'>{df_filtered["risk_score"].mean():.1f}</div>
...
with col4:
    st.markdown(f"""<div class='metric-card gold'>
        ...
        <div class='metric-num' style='color: #C8A86A;'>{df_filtered["hospitalized"].mean() * 100:.1f}%</div>
...
with tab1:
    ...
    with col1:
        ...
        fig = go.Figure(go.Bar(
            ...
            textfont=dict(color="#1E1B18", size=12),
        ))
        fig.update_layout(title="Patients by Risk Tier", xaxis_title=None,
                          yaxis_title="Number of Patients", showlegend=False,
                          xaxis=dict(**AXIS_STYLE), yaxis=dict(**AXIS_STYLE), **PLOTLY_LAYOUT)
    ...
    with col2:
        ...
        fig2 = go.Figure(go.Pie(
            ...
            textinfo="percent+label", textfont=dict(color="#1E1B18", size=11),
        ))
        fig2.update_layout(title="Population Segments", showlegend=False, **PLOTLY_LAYOUT)
        st.plotly_chart(fig2, use_container_width=True)

    st.markdown("**Segment Profiles**")
    seg_profile = df_filtered.groupby("segment_name").agg(
        patients=("age", "count"), avg_age=("age", "mean"),
...
with tab2:
    ...
    fig = go.Figure(go.Bar(
        x=feature_importance["importance"], y=feature_importance["feature_label"],
        orientation="h", marker_color=COLORS["terracotta"],
        text=feature_importance["importance"].round(3), textposition="outside",
        textfont=dict(color="#1E1B18", size=11),
    ))
    fig.update_layout(
        title="Feature Importance for Hospitalization Risk",
        xaxis_title="Importance", yaxis_title=None,
        xaxis=dict(**AXIS_STYLE),
        yaxis=dict(tickfont=dict(color="#1E1B18", size=13), title_font=dict(color="#1E1B18")),
        **PLOTLY_LAYOUT
    )
    st.plotly_chart(fig, use_container_width=True)
    ...
    col1, col2 = st.columns(2)
    with col1:
        fig2 = px.box(df_filtered, x="risk_tier", y="chronic_conditions",
    ...
    with col2:
        fig3 = px.box(df_filtered, x="risk_tier", y="medication_adherence",
                      color="risk_tier", color_discrete_map=RISK_COLORS,
                      category_orders={"risk_tier": ["Low", "Moderate", "High", "Critical"]},
...
with tab3:
    ...
    with col1:
        ...
        fig = go.Figure(go.Bar(
            x=hosp_by_tier.index, y=(hosp_by_tier.values * 100).round(1),
            marker_color=[RISK_COLORS[r] for r in hosp_by_tier.index],
            text=(hosp_by_tier.values * 100).round(1).astype(str) + "%",
            textposition="outside", textfont=dict(color="#1E1B18", size=12),
        ))
        fig.update_layout(title="Avg Hospitalization Probability by Risk Tier",
                          yaxis_title="Probability (%)", xaxis_title=None, showlegend=False,
                          xaxis=dict(**AXIS_STYLE), yaxis=dict(**AXIS_STYLE), **PLOTLY_LAYOUT)
        st.plotly_chart(fig, use_container_width=True)

    with col2:
        fig2 = px.scatter(df_filtered.sample(min(500, len(df_filtered))),
                          x="risk_score", y="hosp_prob", color="risk_tier",
        ...
        fig2.update_layout(xaxis=dict(**AXIS_STYLE), yaxis=dict(**AXIS_STYLE), **PLOTLY_LAYOUT)
        st.plotly_chart(fig2, use_container_width=True)
    ...
    fig3 = px.bar(ed_dist, x="ed_visits_12m", y="count", color="risk_tier",
                  ...
                  category_orders={"risk_tier": ["Low", "Moderate", "High", "Critical"]})
    fig3.update_layout(xaxis=dict(**AXIS_STYLE), yaxis=dict(**AXIS_STYLE), **PLOTLY_LAYOUT)
    st.plotly_chart(fig3, use_container_width=True)
...
with tab4:
    ...
    sdoh_labels = {
        "social_isolation": "Social Isolation", "housing_instability": "Housing Instability",
        "food_insecurity": "Food Insecurity", "transportation_barrier": "Transportation Barrier"
    }
    sdoh_by_tier = df_filtered.groupby("risk_tier")[list(sdoh_labels.keys())].mean() * 100
    sdoh_by_tier.columns = list(sdoh_labels.values())
    sdoh_by_tier = sdoh_by_tier.reindex(["Low", "Moderate", "High", "Critical"])

    fig = go.Figure()
    for factor in sdoh_by_tier.columns:
        fig.add_trace(go.Bar(name=factor, x=sdoh_by_tier.index, y=sdoh_by_tier[factor].round(1)))
    fig.update_layout(barmode="group", title="Social Determinant Prevalence by Risk Tier (%)",
                      yaxis_title="% of Patients Affected", xaxis_title=None,
    ...
    with col1:
        ...
        fig2 = go.Figure(go.Bar(
            x=ins_risk.index, y=ins_risk.values.round(1),
            marker_color=COLORS["terracotta"],
            text=ins_risk.values.round(1), textposition="outside",
            textfont=dict(color="#1E1B18", size=12),
        ))
        fig2.update_layout(title="Average Risk Score by Insurance Type",
                           yaxis_title="Average Risk Score", xaxis_title=None, showlegend=False,
                           xaxis=dict(**AXIS_STYLE), yaxis=dict(**AXIS_STYLE), **PLOTLY_LAYOUT)
        st.plotly_chart(fig2, use_container_width=True)
    ...
    with col2:
        ...
        fig3 = go.Figure(go.Bar(
            x=inc_risk.index, y=inc_risk.values.round(1),
            marker_color=[COLORS["terracotta"], COLORS["gold"], COLORS["sage"]],
            text=inc_risk.values.round(1), textposition="outside",
            textfont=dict(color="#1E1B18", size=12),
        ))
        fig3.update_layout(title="Average Risk Score by Income Level",
                           yaxis_title="Average Risk Score", xaxis_title=None, showlegend=False,
                           xaxis=dict(**AXIS_STYLE), yaxis=dict(**AXIS_STYLE), **PLOTLY_LAYOUT)
        st.plotly_chart(fig3, use_container_width=True)
...
with tab5:
    ...
    st.markdown("""<div style='font-size: 0.87rem; color: #4A4540; line-height: 1.85; max-width: 780px;'>
    ...
    for i, (title, detail) in enumerate(findings):
        st.markdown(f"""<div class='finding-box'>
            <div style='display: flex; gap: 1rem; align-items: flex-start;'>
                ...
                color: #C4714A; line-height: 1; flex-shrink: 0; width: 1.75rem;'>0{i+1}</span>
                <div>
    ...
    st.markdown("""<div style='font-size: 0.82rem; color: #9A9088; margin-top: 1.5rem; padding-top: 1rem;
